...
//         let vec2 = EmbeddingVector::from(vec![4.0, 5.0, 6.0]);
...
//             .await?;
//
//         let query_vec = EmbeddingVector::from(vec![3.0, 1.0, 2.0]);
//         let row = sqlx::query("SELECT embedding FROM sqlx_items ORDER BY embedding <-> $1 LIMIT 1")
//             .bind(query_vec)
//             .fetch_one(&pool)
//             .await?;
//         let res_vec: EmbeddingVector = row.try_get("embedding").unwrap();
//         assert_eq!(vec, res_vec);
//         assert_eq!(vec![1.0, 2.0, 3.0], res_vec.to_vec());
//
//         let empty_vec = EmbeddingVector::from(vec![]);
...
//                 .fetch_one(&pool)
//                 .await?;
//         let null_res: Option<EmbeddingVector> = null_row.try_get("embedding").unwrap();
