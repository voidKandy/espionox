...
use crate::{
    agents::memory::MessageStack, language_models::openai::completions::streaming::StreamResponse,
};
use reqwest::Client;
...
use self::{
    ...
    openai::completions::{
        functions::Function,
        streaming::{CompletionStream, StreamedCompletionHandler},
...
impl LLM {
    ...
    pub fn default_openai(api_key: &str) -> LLM {
        let handler = OpenAiCompletionHandler::default().into();
        LLM::new_completion_model(handler, None, api_key)
    }

    /// Default anthropic handler with 0.7 temp
    pub fn default_anthropic(api_key: &str) -> LLM {
    ...
    pub async fn get_io_completion(
        ...
        let headers = comp_handler.request_headers(&self.api_key);
    ...
    pub async fn get_stream_completion(
        &self,
        messages: &MessageStack,
    ) -> Result<StreamedCompletionHandler, ModelEndpointError> {
        let comp_handler: Box<dyn CompletionEndpointHandler> = self.handler.comp_handler()?;
        let headers = comp_handler.request_headers(&self.api_key);
        ...
        let response_stream: CompletionStream = Box::new(
            ...
            .await
            .map_err(|_| {
                ModelEndpointError::Undefined(anyhow!("Response stream request timed out"))
            })?,
        );
        tracing::info!("Got response stream, returning");
        Ok(response_stream.into())
    }
    ...
    pub async fn get_embedding(&self, text: &str) -> Result<Vec<f32>, ModelEndpointError> {
        let emb_handler: Box<dyn EmbeddingEndpointHandler> = self.handler.emb_handler()?;
        let headers = emb_handler.request_headers(&self.api_key);
