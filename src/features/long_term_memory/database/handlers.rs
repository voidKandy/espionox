use crate::memory::embeddings::EmbeddingVector;

use super::init::DbPool;
use super::models::*;
use sqlx::postgres::PgQueryResult;

pub async fn get_message_by_id(pool: &DbPool, id: &str) -> anyhow::Result<SqlMessage> {
    let query = sqlx::query_as::<_, SqlMessage>("SELECT * FROM messages WHERE id = $1");
    match query.bind(id).fetch_one(pool.as_ref()).await {
        Ok(result) => Ok(result),
        Err(err) => Err(err.into()),
    }
}

pub async fn get_metadata_by_id(pool: &DbPool, id: &str) -> anyhow::Result<SqlMetadata> {
    let query = sqlx::query_as::<_, SqlMetadata>("SELECT * FROM metadata WHERE id = $1");
    match query.bind(id).fetch_one(pool.as_ref()).await {
        Ok(result) => Ok(result),
        Err(err) => Err(err.into()),
    }
...
pub async fn get_infos_by_id(pool: &DbPool, parent_id: &str) -> anyhow::Result<Vec<SqlInfo>> {
    let query = sqlx::query_as::<_, SqlInfo>("SELECT * FROM info WHERE parent_id = $1");
    match query.bind(parent_id).fetch_all(pool.as_ref()).await {
        Ok(result) => Ok(result),
        Err(err) => Err(err.into()),
    }
}
